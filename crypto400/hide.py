import sys, struct
K = []
S = []
R = [ 0 ] * 16
T = []

# XOR a with b as key, rotate over b if needed
def string_xor(a,key):
    return ''.join(chr(ord(a[i]) ^ ord(key[i%len(key)])) for i in range(len(a)))

# Plain RC4 setup with a random 16 byte key K
def setup():
    global K
    global S
    global R
    K = map(ord, open('/tmp/urandom').read(16))
    S = [i for i in range(256)]
    j = 0
    for i in range(256):
        j = (K[i % len(K)] + S[i] + j) % 256
        S[i], S[j] = S[j], S[i]
    for k in range(16):
        R[k] = K[k]

# Clock function
def clock():
    new = K[6] ^ T[K[15]] ^ K[9]
    i = 15
    while i > 0:
        K[i] = K[i-1]
        i -= 1
    K[0] = new

def get_random(l):
    output = []
    setup()
    for v in range(l):
        clock()
        i = K[4]
        j = K[10]
        print v, i, j
        output.append(S[(S[j] + S[i]) % 256])
        S[i], S[j] = S[j], S[i]
    return output

flag = map(ord, sys.argv[1])
outfile = sys.argv[2]

r = get_random(8192)
pos = struct.unpack("<L", open("/tmp/urandom").read(4))[0] % (8192 - len(flag))

for (i,o) in enumerate(range(pos,pos+len(flag))):
    r[o] ^= flag[i]

out = open(outfile, 'wb')
out.write(''.join(map(chr, r)))
